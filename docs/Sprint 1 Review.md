...
## 1. Narrative: Completed Work & Design Decisions
  ...
  Feature Implementation: We successfully established the Web Scraping connection to gather initial data from JCDecaux and OpenWeather.
  
  Design Decisions: Based on Competitors Research, we decided to prioritize features such as live availability and weather integration to address specific user pain points.
  
  UX/UI Strategy: We defined User Stories and translated them into high-fidelity Mockups to visualize the dashboard and map interface before beginning the core coding phase.
...
## 2. Burndown Chart
  ...
  Trend Analysis: Although the first few days showed a slower decline due to the learning curve of the project requirements, the Actual Remaining work eventually tracked closely with the Idea time slope as the team became more organized.
...
## 3. Sprint Review
  ...
  Functional Scraper: A working script capable of extracting real-time station data.
  
  Interactive Mockups: A visual walkthrough of the user journey for our three primary personas: Liam, Sarah, and Thomas.
